...
//        config.setPassword(RedisPassword.of(secondPassword));
    ...
    //配置消息监听器  只有rus订阅了 这一些频道
    @Bean(name = "sessionContainer")
    public RedisMessageListenerContainer container(@Qualifier("sessionInfoFactory") LettuceConnectionFactory redisConnectionFactory, @Qualifier("RedisMessageListener") RedisMessageListener messageListener) {
        RedisMessageListenerContainer container = new RedisMessageListenerContainer();
        container.setConnectionFactory(redisConnectionFactory);
        container.addMessageListener(messageListener, new ChannelTopic("my_channel"));
        container.addMessageListener(messageListener, new ChannelTopic("setting"));
        container.addMessageListener(messageListener, new ChannelTopic("notice_msg"));
        return container;
    }
